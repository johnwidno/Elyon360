        ...
        <AdminLayout>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                <div>
                    ...
                    <p className="text-app-meta text-gray-500 dark:text-gray-400 mt-2 font-medium">
                        {t('resources_desc', 'Suivez l\'état et la localisation de vos équipements.')}
                    </p>
                </div>
                <button onClick={() => setView('add')} className="px-6 py-3 bg-brand-primary hover:bg-brand-deep text-white font-black uppercase tracking-widest text-app-micro rounded-2xl transition-all shadow-lg active:scale-95">
            ...
            {view === 'add' && (
                <div className="bg-white dark:bg-[#1A1A1A] p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-sm mb-8 animate-fade-in">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6 max-w-lg">
                        <div>
                            <label className="block text-app-micro font-bold text-gray-700 dark:text-gray-300 mb-2">{t('name', 'Nom')}</label>
                        ...
                        <div>
                            <label className="block text-app-micro font-bold text-gray-700 dark:text-gray-300 mb-2">{t('room_location', 'Salle / Emplacement')}</label>
                        ...
                        <div className="flex gap-4">
                            <div className="w-1/2">
                                <label className="block text-app-micro font-bold text-gray-700 dark:text-gray-300 mb-2">{t('quantity', 'Quantité')}</label>
                                <input type="number" className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 outline-none transition-all text-app-meta"
                                    value={form.quantity} onChange={e => setForm({ ...form, quantity: e.target.value })} required />
                            </div>
                            <div className="w-1/2">
                                <label className="block text-app-micro font-bold text-gray-700 dark:text-gray-300 mb-2">{t('status', 'État')}</label>
                                <select className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 outline-none transition-all text-app-meta"
                        ...
                        <div className="flex gap-3 pt-4">
                            <button type="submit" className="px-6 py-3 bg-brand-primary hover:bg-brand-deep text-white font-black uppercase tracking-widest text-app-micro rounded-xl transition-all active:scale-95">{t('save', 'Enregistrer')}</button>
                            <button type="button" onClick={() => setView('list')} className="px-6 py-3 bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 font-black uppercase tracking-widest text-app-micro rounded-xl hover:bg-gray-200 dark:hover:bg-white/10 transition-colors">{t('cancel', 'Annuler')}</button>
                        </div>
                    </form>
                </div>
            )}
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-gray-100 dark:border-white/5">
                                <th className="pb-4 text-app-micro font-bold text-gray-400 uppercase tracking-widest">{t('item_name', 'Nom')}</th>
                                <th className="pb-4 text-app-micro font-bold text-gray-400 uppercase tracking-widest">{t('location', 'Lieu')}</th>
                                <th className="pb-4 text-app-micro font-bold text-gray-400 uppercase tracking-widest">{t('quantity', 'Quantité')}</th>
                                <th className="pb-4 text-app-micro font-bold text-gray-400 uppercase tracking-widest">{t('status', 'État')}</th>
                                <th className="pb-4 text-right text-app-micro font-bold text-gray-400 uppercase tracking-widest">{t('actions')}</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                            {resources.length > 0 ? resources.map(item => (
                                <tr key={item.id} className="group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="py-4 text-app-meta font-medium text-gray-900 dark:text-white">
                                        {rooms.find(r => r.id === item.roomId)?.name || 'N/A'}
                                    </td>
                                    <td className="py-4 text-app-meta text-gray-500 dark:text-gray-400">{item.quantity}</td>
                                    <td className="py-4">
                                        <span className={`px-2 py-1 text-app-micro font-bold rounded-lg uppercase tracking-widest 
                                    ...
                                    <td className="py-4 text-right">
                                        <button className="text-gray-400 hover:text-brand-primary dark:hover:text-brand-orange transition-colors font-bold text-app-micro uppercase tracking-wider">{t('edit', 'Modifier')}</button>
                                    </td>
                                </tr>
                            )) : (
                                <tr>
