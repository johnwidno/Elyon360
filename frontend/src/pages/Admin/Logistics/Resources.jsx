        ...
        <AdminLayout>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                <div>
                    <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">
                        {t('resources', 'Matériels')} <span className="text-indigo-600 dark:text-indigo-400">& {t('inventory', 'Inventaire')}</span>
                    </h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
                        {t('resources_desc', 'Suivez l\'état et la localisation de vos équipements.')}
                    </p>
                </div>
                <button onClick={() => setView('add')} className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl transition-all shadow-lg shadow-indigo-200 dark:shadow-none active:scale-95">
            ...
            {view === 'add' && (
                <div className="bg-white dark:bg-[#1A1A1A] p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-sm mb-8 animate-fade-in">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Ajouter du Matériel</h2>
                    <form onSubmit={handleSubmit} className="space-y-6 max-w-lg">
                        <div>
                            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Nom</label>
                            <input type="text" className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all"
                                value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Salle / Emplacement</label>
                            <select className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all"
                        ...
                        <div className="flex gap-4">
                            <div className="w-1/2">
                                ...
                                <input type="number" className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all"
                                    value={form.quantity} onChange={e => setForm({ ...form, quantity: e.target.value })} required />
                            </div>
                            <div className="w-1/2">
                                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">État</label>
                                <select className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all"
                        ...
                        <div className="flex gap-3 pt-4">
                            <button type="submit" className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors">Enregistrer</button>
                            <button type="button" onClick={() => setView('list')} className="px-6 py-3 bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 font-bold rounded-xl hover:bg-gray-200 dark:hover:bg-white/10 transition-colors">Annuler</button>
                        </div>
                    </form>
                </div>
            )}
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                            {resources.length > 0 ? resources.map(item => (
                                <tr key={item.id} className="group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                    <td className="py-4 font-bold text-gray-900 dark:text-white group-hover:text-indigo-600 transition-colors">{item.name}</td>
                                    ...
                                    <td className="py-4">
                                        <span className={`px-2 py-1 text-[10px] font-bold rounded-lg uppercase tracking-widest 
                                            ...
                                            {item.status === 'bon_etat' ? 'Bon état' : item.status === 'reparer' ? 'À réparer' : 'Hors service'}
                                        </span>
                                    </td>
                                    <td className="py-4 text-right">
                                        <button className="text-gray-400 hover:text-indigo-600 transition-colors">Modifier</button>
                                    </td>
                                </tr>
                            )) : (
                                <tr>
                                    <td colSpan="5" className="py-8 text-center text-gray-400 italic">Aucun matériel trouvé.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
