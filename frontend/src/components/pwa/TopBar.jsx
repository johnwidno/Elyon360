...
const TopBar = ({ title, onMenuClick, centerContent }) => {
  const { language, setLanguage } = useLanguage();
  const { theme, toggleTheme } = useTheme();

  return (
    <header className="fixed top-0 left-0 right-0 h-16 md:h-20 bg-white dark:bg-slate-900 border-b border-slate-50 dark:border-slate-800 z-50 flex items-center transition-all px-6">
      <div className="max-w-screen-2xl mx-auto w-full flex items-center justify-between">
        ...
        <div id="topbar-center-target" className="hidden md:flex flex-1 items-center justify-center px-8">
          {centerContent}
        </div>
