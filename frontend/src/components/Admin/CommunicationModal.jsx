...
export default function CommunicationModal({
    ...
    defaultContent = "Bonjour {firstName}, "
}) {
    const { t } = useLanguage();
    ...
    // Update content when defaults change or recipient changes
    useEffect(() => {
        if (isOpen) {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setStatus({
                show: true,
                message: mode === 'bulk' ? t('messages_sent_all', "Messages envoyés à tous !") : t('message_sent', "Message envoyé !"),
                type: 'success'
            });
            ...
            setStatus({ show: true, message: t('send_error', "Erreur lors de l'envoi."), type: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm transition-opacity">
            <div className="bg-white dark:bg-[#1A1A1A] rounded-[30px] w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 border border-white/5">
                <div className="p-8">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                            {mode === 'bulk'
                                ? t('send_bulk_message', "Envoyer un message groupé")
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 px-1">{t('subject_title', 'Objet / Titre')}</label>
                            <input
                                type="text"
                                className="w-full px-5 py-3.5 bg-gray-50 dark:bg-black border border-gray-100 dark:border-white/5 rounded-xl text-[13px] font-medium text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500/20 transition-all shadow-sm"
                                value={title}
                        ...
                        <div>
                            <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 px-1">{t('message', 'Message')}</label>
                            <textarea
                                className="w-full px-5 py-4 bg-gray-50 dark:bg-black border border-gray-100 dark:border-white/5 rounded-xl text-[13px] font-medium text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500/20 transition-all h-32 resize-none shadow-sm"
                                value={content}
                                onChange={(e) => setContent(e.target.value)}
                            />
                            <p className="mt-2 text-[10px] text-gray-500 px-1 italic">
                                {t('placeholder_hint', 'Astuce: Utilisez {firstName} pour personnaliser automatiquement le prénom.')}
                            </p>
                        </div>

                        {mode === 'bulk' && (
                            <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-white/5">
                                <p className="text-[11px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-1">{t('recipients', 'Destinataires')}</p>
                                <p className="text-[12px] text-blue-700 dark:text-blue-300 font-medium">
                                    {recipients.length} {t('members_targeted', 'membres ciblés')}.
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="mt-8 flex gap-4">
                        ...
                        >
                            {loading ? t('sending', "Envoi...") : (mode === 'bulk' ? t('broadcast_all', "Diffuser à tous") : t('send', "Envoyer"))}
                        </button>
                    </div>
                </div>
            </div>
